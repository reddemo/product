...
;(function(y, root, $) {
    function Form() {
        ...
        var http = y.throttle(_http, 3000);

        function init() {
            listenToDom();
        }

        function checks(xs) { //生成整个表单的校验逻辑
        ...
        function validators(rules, field) { //生成输入域的校验逻辑
            var arr=(y.cat(y.keys(fields[field.name].rules),y.keys(field.common)));
            ...
            }, y.find(arr,'alt')?[]:[y.validator(required.predicator, required.error, required.failure || false, '')]);
        ...
        function listenToDom(form) { //监听提交事件
            $submit.on('click', function() {
                try {
                    ...
                    if (y.isString(r)) {
                        alert(['数据校验失败了，理由如下', r].join(':'));
                    } else {
                        http(r,conf.token,conf.id);
                    }
                    return false;
                }catch(e){
                    return false;
        ...
        function row(predictors, val, text) { //校验每个属性，如果失败就短路
        ...
        function col(checks, vals) { //校验每个filed，如果失败就短路
